...
if ($rows && isset($rows['name']) && isset($rows['billno']) && isset($rows['amount'])) {
    echo "Name: " . $rows['name'] . "<br>";
    echo "Bill NO: " . $rows['billno'] . "<br>";
    echo "Amount: " . $rows['amount'] . "<br>";
    $amount = $rows['amount'];
} else {
    echo "Error: Unable to fetch bill information.";
    // Optionally, handle the case where the required fields are not available
